  ...
  async extractData(page) {
    ...
    try {
      return {
        ...
        phones: content.match('Phone Numbers') ? (await page.$$eval(`div#personDetails > div:nth-child(${this.next(true)}) > div:nth-child(2) div.content-value`, (els) => els.map((v) => v.innerText.trim()))).map(TruePeopleSearch.phoneToObject) : [],
        emails: content.match('mails') ? await page.$$eval(`div#personDetails > div:nth-child(${this.next(true)}) > div:nth-child(2) div.content-value`, (els) => els.map((v) => v.innerText.trim())) : [],
        names: content.match('Names') ? await page.$eval(`div#personDetails > div:nth-child(${this.next(true)}) div.content-value`, (el) => el.innerText.trim().split(',').map((v) => v.trim())) : [],
        addresses: content.match('Previous Addresses') ? (await page.$$eval(`div#personDetails > div:nth-child(${this.next(true)}) div.content-value`, (els) => els.map((v) => v.innerText.trim()))).map(TruePeopleSearch.formatAddress) : [],
        relatives: content.match('Relatives') ? await page.$eval(`div#personDetails > div:nth-child(${this.next(true)}) div.content-value`, (el) => el.innerText.trim().split(',').map((v) => v.trim())) : [],
        associates: content.match('Associates') ? await page.$eval(`div#personDetails > div:nth-child(${this.next(true)}) div.content-value`, (el) => el.innerText.trim().split(',').map((v) => v.trim())) : []
      }
    } catch (e) {
